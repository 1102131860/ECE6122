...
glm::mat4 ViewMatrix = glm::lookAt(
	glm::vec3(cRadius * sin(glm::radians(cTheta)) * cos(glm::radians(cPhi)),
		cRadius * sin(glm::radians(cTheta)) * sin(glm::radians(cPhi)),
		cRadius * cos(glm::radians(cTheta))),           // Camera is here
...
{
	if (splitedCmd.size() == 2 && splitedCmd[0] == "power") {
		float intensity;
		std::stringstream ss(splitedCmd[1]); // 100.f
		ss >> intensity;
		if (ss.fail() || intensity < 0.f) {
			return false;
		}
	...
	else if (splitedCmd.size() == 4) {
		float theta, phi, radius;
		std::stringstream ss1(splitedCmd[1]);
		std::stringstream ss2(splitedCmd[2]);
		std::stringstream ss3(splitedCmd[3]);
		ss1 >> theta;
		ss2 >> phi;
		ss3 >> radius;
		if (ss1.fail() || ss2.fail() || ss3.fail() ||
			(theta < 10.f || theta > 80.f) || (phi < 0.f || phi > 360.f) || radius <= 0.f) {
			return false;
		}

		// Convert to Cartesian co-ordinate system
		float posX = radius * sin(glm::radians(theta)) * cos(glm::radians(phi));
		float posY = radius * sin(glm::radians(theta)) * sin(glm::radians(phi));
		float posZ = radius * cos(glm::radians(theta));

		if (splitedCmd[0] == "camera") {
			// Pass the theta, phi, radius to the set camera position function
			// Update ViewMatrix and ProjectionMatrix
			...
			ViewMatrix = glm::lookAt(
				position,           // Camera is here
				glm::vec3(0, 0, 0),             // and looks here : at the same position, plus "direction", look at origin
				glm::vec3(0, 0, 1)              // Look in the z-direction (set to 0,0,1 to look upside-down)
			);
			return true;
		}
		else if (splitedCmd[0] == "light") {
			// Pass the theta, phi, radius to the set light position function
